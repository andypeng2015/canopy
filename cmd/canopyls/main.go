...
func main() {
	if len(os.Args) > 1 && os.Args[1] == "--version" {
		fmt.Println("canopyls " + version)
		os.Exit(0)
	}

	// Client mode: canopyls client <method> [params...]
	...
	if len(os.Args) > 1 && os.Args[1] != "--stdio" {
		...
		if err := lsp.RunDriver(cwd, os.Args[1:]); err != nil {
			fmt.Fprintf(os.Stderr, "canopyls driver: %v\n", err)
			os.Exit(1)
		}
		return
	}
	...
	if err := srv.Serve(); err != nil {
		fmt.Fprintf(os.Stderr, "canopyls: %v\n", err)
		os.Exit(1)
	}
}
...
func runClient(args []string) {
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr, "usage: canopyls client <method> [param=value ...]")
		os.Exit(1)
	}
	method := args[0]
